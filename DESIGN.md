...
- **LSP efímero** (`runtime --lsp`): `diagnostics` + `resolved_type` por símbolo y tipos por
  **parámetro** (M4b). **Multi-lenguaje** (M4 + M11a/b): Python (pyright/pylsp/jedi) y TS/JS
  ...
  **Grupo B (M11b):** Java (`jdtls`) — necesita init a medida (workspace `-data <dir>` propio,
  que el cliente crea y limpia; JVM/JDK 17+). Todos aportan diagnósticos + `resolved_type`; su
  server es toolchain/OS-específico, así que `doctor` lo detecta y muestra cómo instalarlo (no
  lo auto-instala). El resto que MemoryGraf indexa (C#, PHP, R, VB, asm) sigue symbols-only.
